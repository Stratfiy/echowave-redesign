...
The generated files live under `decibyl_sdk.typed` and are committed to
the repository so `pip install decibyl-sdk` ships typed classes without
requiring a regen step.

Run manually:

    python -m decibyl_sdk.codegen --api http://localhost:8000 \\
        --out sdk/python/src/decibyl_sdk/typed

    python -m decibyl_sdk.codegen --input specs.json \\
        --out ./my_typed
"""
...
def _render_init_module(spec_names: list[str]) -> str:
    lines = [
        ...
        "`from decibyl_sdk.typed import StartCall, AgentNode`.",
        '"""',
        "",
    ]
    exports: list[str] = []
    for spec_name in sorted(spec_names):
        ...
        lines.append(f"from decibyl_sdk.typed.{module_name} import {class_name}")
        exports.append(class_name)

    lines.append("from decibyl_sdk.typed._base import TypedNode")
...
def main(argv: list[str] | None = None) -> None:
    parser = argparse.ArgumentParser(
        prog="python -m decibyl_sdk.codegen",
        description="Generate typed SDK dataclasses from the Decibyl node-spec catalog.",
    )
    source = parser.add_mutually_exclusive_group(required=True)
    source.add_argument("--api", help="Decibyl backend base URL")
