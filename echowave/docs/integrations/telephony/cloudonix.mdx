...
2. Create a new application (or open the existing one whose name you configured in Decibyl) with these settings:
   ...
   - **Application URL**: `https://api.decibyl.ai/api/v1/telephony/inbound/run`
   - **HTTP Method**: `POST`
...
3. Confirm:
   - **URL** is set to: `https://api.decibyl.ai/api/v1/telephony/inbound/run`
   - **Method** is `POST`
   ...
   <Note>
     ...
     self-hosted Decibyl, replace `api.decibyl.ai` with your backend domain.
   </Note>
