...
```
wss://api.decibyl.ai/api/v1/agent-stream/{provider}/{agent_uuid}
```

`{provider}` is the registered provider name, currently `cloudonix`. `{agent_uuid}` is the agent's stable UUID (see [Get the Agent UUID](#get-the-agent-uuid) below). On self-hosted deployments, replace `api.decibyl.ai` with your backend host.
...
```
wss://api.decibyl.ai/api/v1/agent-stream/cloudonix/{agent_uuid}
```
...
<Response>
  <Connect>
    <Stream url="wss://api.decibyl.ai/api/v1/agent-stream/cloudonix/{agent_uuid}"/>
  </Connect>
  <Pause length="40"/>
</Response>
```
