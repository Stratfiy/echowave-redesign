...
description: "Context variables available in webhook nodes and the data EchoWave sends after a call"
---

EchoWave executes **webhook nodes** asynchronously after a workflow run completes. You configure the target URL, HTTP method, headers, and payload template directly in the workflow definition.
...
2. EchoWave executes any `webhook` nodes in the workflow asynchronously
...
@app.post("/webhook/echowave")
async def handle_echowave_webhook(request: Request):
...
```json
{
  ...
  "data": {
    ...
    "enabled": true,
    "http_method": "POST",
    "endpoint_url": "https://your-service.com/webhook/echowave",
    "payload_template": {
      "run_id": "{{workflow_run_id}}",
      "customer": "{{initial_context.customer_name}}",
      "outcome": "{{gathered_context.resolution}}"
