---
title: Setup
description: Get a EchoWave dev environment running and learn the daily commands.
---

EchoWave is a Next.js UI (`ui/`), a FastAPI backend (`api/`), and a [Pipecat](https://github.com/pipecat-ai/pipecat)-based voice pipeline (`pipecat/`, a git submodule), backed by Postgres, Redis, and MinIO.

<Tip>
**Using Claude Code or Codex?** Install the [EchoWave setup skill](https://github.com/echowave-hq/echowave-plugins) (`/plugin marketplace add echowave-hq/echowave-plugins`, then `/plugin install echowave@echowave`) and ask your agent to *"set up EchoWave for development"* — it runs the steps below for you and verifies the stack is healthy.
</Tip>
...
1. Fork [echowave-hq/echowave](https://github.com/echowave-hq/echowave) and clone **your fork**:

```bash
git clone https://github.com/<YOUR_HANDLE>/echowave
cd echowave
```
...
If these steps do not work for you, please open an issue on [GitHub](https://github.com/echowave-hq/echowave/issues).

<AccordionGroup>
<Accordion title="Cloned echowave-hq/echowave directly instead of your fork?">
Run `bash scripts/setup_fork.sh` once. It prompts for your fork URL, points `origin` at it, and adds `upstream` as `echowave-hq/echowave`. See [Fork and upstream remotes](/contribution/reference#fork-and-upstream-remotes).
</Accordion>
...
| `sdk/` | Python/TypeScript SDKs for driving EchoWave programmatically |
...
2. Push to your fork (`origin`) and open a pull request against `echowave-hq/echowave:main`.
3. A maintainer reviews and merges.

Bug reports and feature ideas go through [GitHub Issues](https://github.com/echowave-hq/echowave/issues) and [Ideas](https://github.com/orgs/echowave-hq/discussions/categories/ideas). Issues tagged `good first issue` are a good place to start. For questions while you're working, use the [EchoWave Community Slack](https://join.slack.com/t/echowave-community/shared_invite/zt-3zjb5vwvl-j7hRz3_F1SOn5cH~jm5f5g).
