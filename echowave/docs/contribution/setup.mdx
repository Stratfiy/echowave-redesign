---
title: Setup
description: Get a Decibyl dev environment running and learn the daily commands.
---

Decibyl is a Next.js UI (`ui/`), a FastAPI backend (`api/`), and a [Pipecat](https://github.com/pipecat-ai/pipecat)-based voice pipeline (`pipecat/`, a git submodule), backed by Postgres, Redis, and MinIO.

<Tip>
**Using Claude Code or Codex?** Install the [Decibyl setup skill](https://github.com/decibyl-hq/decibyl-plugins) (`/plugin marketplace add decibyl-hq/decibyl-plugins`, then `/plugin install decibyl@decibyl`) and ask your agent to *"set up Decibyl for development"* — it runs the steps below for you and verifies the stack is healthy.
</Tip>
...
1. Fork [decibyl-hq/decibyl](https://github.com/decibyl-hq/decibyl) and clone **your fork**:

```bash
git clone https://github.com/<YOUR_HANDLE>/decibyl
cd decibyl
```
...
If these steps do not work for you, please open an issue on [GitHub](https://github.com/decibyl-hq/decibyl/issues).

<AccordionGroup>
<Accordion title="Cloned decibyl-hq/decibyl directly instead of your fork?">
Run `bash scripts/setup_fork.sh` once. It prompts for your fork URL, points `origin` at it, and adds `upstream` as `decibyl-hq/decibyl`. See [Fork and upstream remotes](/contribution/reference#fork-and-upstream-remotes).
</Accordion>
...
| `sdk/` | Python/TypeScript SDKs for driving Decibyl programmatically |
...
2. Push to your fork (`origin`) and open a pull request against `decibyl-hq/decibyl:main`.
3. A maintainer reviews and merges.

Bug reports and feature ideas go through [GitHub Issues](https://github.com/decibyl-hq/decibyl/issues) and [Ideas](https://github.com/orgs/decibyl-hq/discussions/categories/ideas). Issues tagged `good first issue` are a good place to start. For questions while you're working, use the [Decibyl Community Slack](https://join.slack.com/t/decibyl-community/shared_invite/zt-3zjb5vwvl-j7hRz3_F1SOn5cH~jm5f5g).
