...
1. Run the contributor bootstrap. It configures `origin` as your fork and `upstream` as `echowave-hq/echowave`, initializes the pipecat submodule, creates the Python venv, and copies the `.env` templates:
...
The checked-in `.devcontainer/` follows the Dev Containers specification, so it should also work in Cursor, JetBrains IDEs, and other compatible editors. Only the VS Code path is actively tested — if something breaks elsewhere, please open a [GitHub issue](https://github.com/echowave-hq/echowave/issues).
...
- `upstream`: `echowave-hq/echowave`, where new commits land

If you cloned `echowave-hq/echowave` directly instead of your fork, run it once (inside the devcontainer is fine):
...
```bash
origin    https://github.com/<YOUR_HANDLE>/echowave.git (fetch/push)
upstream  https://github.com/echowave-hq/echowave.git    (fetch/push)
```

<Note>
Always push feature branches to **`origin`** (your fork), then open a pull request against `echowave-hq/echowave:main`. Never push directly to `upstream`.
</Note>
