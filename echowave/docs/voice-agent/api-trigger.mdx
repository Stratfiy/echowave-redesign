...
If you are using the hosted version, replace `your-decibyl-instance` with `api.decibyl.ai`.
...
The id is shown on each row in **Telephony configurations** (`https://app.decibyl.ai/telephony-configurations` for hosted or `http://localhost:3010/telephony-configurations` for local). The configuration must belong to the same organization as the API Trigger; otherwise the request returns `404`.
