...
The API Trigger node lets you initiate outbound calls to your voice agent programmatically. When you add an API Trigger node to your workflow, Decibyl generates a unique endpoint URL that external systems can call to start a conversation.
...
When you add an API Trigger node to your workflow, Decibyl assigns it a unique UUID. The trigger node exposes two URLs that share this UUID:
...
```
POST https://api.decibyl.ai/api/v1/public/agent/{uuid}        # Production
POST https://api.decibyl.ai/api/v1/public/agent/test/{uuid}   # Test
```

Every account calls the same host.
...
<CodeGroup>
```bash Production URL
curl -X POST https://api.decibyl.ai/api/v1/public/agent/{uuid} \
...
A successful request returns a `workflow_run_id` that you can use to [retrieve run details](/api-reference/calls/get), recordings, and transcripts.
...
The id is shown on each row in **Telephony configurations** ([app.decibyl.ai/telephony-configurations](https://app.decibyl.ai/telephony-configurations)). The configuration must belong to the same organization as the API Trigger; otherwise the request returns `404`.

<Note>
For full endpoint details including all parameters and response fields, see the [API reference](/api-reference/calls/start).
</Note>
