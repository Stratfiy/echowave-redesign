...
When the LLM decides a transfer is needed, it calls the Call Transfer tool. EchoWave resolves the destination, validates the transfer setup, and then starts the provider transfer.

For a successful transfer:

1. The agent calls the Call Transfer tool.
2. EchoWave resolves the destination from either static configuration or a dynamic HTTP resolver.
3. EchoWave optionally plays a pre-transfer message.
4. EchoWave starts dialing the destination through the telephony provider.
5. The caller hears hold music while the destination leg is connecting.
6. When the destination answers, EchoWave connects the caller and ends the AI agent's involvement.

The transfer is a blind transfer. EchoWave does not currently pass conversation context to the receiving human as part of the transfer.
...
EchoWave sends a `POST` request to your resolver endpoint before starting the transfer. Your endpoint returns the destination and may return a custom message to play before dialing.
...
- Use business logic from your backend instead of hardcoding numbers in EchoWave

## Dynamic resolver request

The resolver request body is a flat JSON object.

EchoWave builds it from:

- **LLM parameters**: values the agent extracts from the conversation when it calls the transfer tool
- **Preset parameters**: values EchoWave injects from fixed values or templates such as `{{initial_context.account_id}}` or `{{gathered_context.billing_issue_type}}`
...
<Note>
EchoWave does not send the full conversation transcript to the resolver. Add the specific values you need as LLM parameters or preset parameters.
</Note>
...
<Warning>
Follow this response shape exactly. EchoWave reads specific keys from the resolver response. For dynamic transfer resolution, an invalid response does not fall back silently: the transfer fails before dialing.
</Warning>
...
- **Resolver URL**: HTTPS or HTTP endpoint EchoWave calls with `POST`
- **Resolver Timeout**: How long EchoWave waits for the resolver response
- **Resolver Wait Message**: Optional message spoken while EchoWave waits, such as "One moment while I find the right team."
- **LLM Parameters**: Values the agent should extract from the conversation, such as `billing_issue_type` or `requested_department`
- **Preset Parameters**: Values injected by EchoWave from context or fixed values
...
1. EchoWave resolves the static or template destination.
2. EchoWave plays the configured pre-transfer message, if any.
3. EchoWave starts the provider transfer.

For dynamic transfers:

1. EchoWave may play the resolver wait message while calling your resolver.
2. If the resolver returns `custom_message`, EchoWave plays that message.
3. If the resolver does not return `custom_message`, EchoWave uses the configured pre-transfer message.
4. EchoWave starts the provider transfer.

Resolver `custom_message` overrides the configured pre-transfer message for that transfer attempt.

## Transfer timeout

Transfer timeout is the maximum time EchoWave waits for the destination to answer after the provider transfer starts.

This timeout applies to both static and dynamic transfers.

The resolver timeout is separate. It controls only how long EchoWave waits for your HTTP resolver before dialing starts.
...
- a preset parameter, if EchoWave should inject it from context
