...
| Field | Description |
| --- | --- |
| **Endpoint URL** | The URL EchoWave will send the POST request to. |
...
EchoWave sends a `POST` request with the following JSON payload:
...
```javascript
app.post("/echowave/pre-call", async (req, res) => {
