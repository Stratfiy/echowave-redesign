...
description: "Trigger a EchoWave voice agent to call a phone number from the SDK"
---

Use the SDK to place a test outbound call from a specific agent to a phone number. This is the same endpoint used by the **Test Call** button in the EchoWave UI.

## Prerequisites

- A EchoWave [API key](/configurations/api-keys) exported as `ECHOWAVE_API_KEY`
...
from echowave_sdk._generated_models import InitiateCallRequest

with EchoWaveClient(api_key="YOUR_API_KEY") as client:
...
const client = new EchoWaveClient({ apiKey: "YOUR_API_KEY" });
