...
description: "Configure the speech-to-speech, Decibyl-managed, or bring-your-own-key models your Decibyl agents use."
---
...
To configure models, open **Models** in your Decibyl dashboard:
...
| **Decibyl** | Use Decibyl-managed LLM, voice, and transcriber models behind one Decibyl Service Key. |
...
<Screenshot
  src="model-configuration-speech-to-speech.png"
  alt="Model configuration on the speech-to-speech arrangement"
  caption="Speech to speech: one realtime model handles the conversation."
  shot="Model configuration showing the speech-to-speech arrangement in the CURRENT UI. The old three-tab strip (Speech to Speech / Dograh / BYOK) no longer exists, so this needs recapturing against the redesigned screen — and the surrounding prose on this page may need rewriting to match."
/>
...
<Warning>
An LLM is still required when you use Speech to Speech. The realtime model handles the live voice conversation, but Decibyl uses the LLM for analysis tasks that happen outside the live audio stream.
</Warning>

## Decibyl

Use **Decibyl** when you want Decibyl to manage the model providers for you. This path uses one Decibyl Service Key for Decibyl-managed models instead of separate provider keys for LLM, Voice, and Transcriber.

<Screenshot
  src="model-configuration-managed.png"
  alt="Model configuration on managed Decibyl models"
  caption="Managed: nothing to configure but the voice and the language."
  shot="Model configuration showing the managed-models arrangement in the CURRENT UI. The tab in the old image is literally labelled 'Dograh'. Recapture against the redesigned screen."
/>
...
<Screenshot
  src="model-configuration-byok.png"
  alt="Model configuration with your own provider keys"
  caption="Bring your own key: pick the exact provider and model, and paste the key."
  shot="Model configuration showing the BYOK arrangement in the CURRENT UI. Same redesign caveat as the other two on this page."
/>
