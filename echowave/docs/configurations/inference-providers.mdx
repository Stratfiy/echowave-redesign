...
description: "Configure the speech-to-speech, EchoWave-managed, or bring-your-own-key models your EchoWave agents use."
---
...
To configure models, open **Models** in your EchoWave dashboard:

- **Hosted:** `https://app.echowave.com/model-configurations`
...
| **EchoWave** | Use EchoWave-managed LLM, voice, and transcriber models behind one EchoWave Service Key. |
...
<Warning>
An LLM is still required when you use Speech to Speech. The realtime model handles the live voice conversation, but EchoWave uses the LLM for analysis tasks that happen outside the live audio stream.
</Warning>

## EchoWave

Use **EchoWave** when you want EchoWave to manage the model providers for you. This path uses one EchoWave Service Key for EchoWave-managed models instead of separate provider keys for LLM, Voice, and Transcriber.

![EchoWave model configuration](../images/model-configuration-echowave.png)

Configure:

| Field | What it controls |
|-------|------------------|
| **Voice** | The EchoWave-managed voice to use. |
| **Speed** | The voice playback speed. |
| **Language** | The language behavior, including multilingual auto-detect when available. |
| **API Key** | Your EchoWave Service Key. Create Service Keys from **Developers**. |
