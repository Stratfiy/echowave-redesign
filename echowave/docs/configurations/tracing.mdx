...
<Screenshot
  src="accessing-traces.png"
  alt="A completed agent run with the View Trace button beside the transcript and recording downloads"
  caption="View Trace, on a completed run."
  shot="A completed run panel showing Preview / Download / Trace controls. The Gathered Context JSON is visible in this shot — make sure the trace_url in it points at the Decibyl tracing host, not langfuse.dograh.com."
/>
...
[Langfuse](https://langfuse.com/docs) is an open-source LLM observability platform — it stores and visualizes traces (prompts, responses, tool calls) so you can debug and iterate on LLM-powered applications outside of Decibyl's own trace viewer.

We provide seamless integration with Langfuse for tracing if you want to use your own account. This enables you to use the [playground feature of Langfuse](https://langfuse.com/docs/prompt-management/features/playground). This works on both managed and self-hosted Decibyl deployments.

**Setup steps:**

1. Sign up at [Langfuse](https://langfuse.com) and create API credentials
2. In the Decibyl UI, go to **Platform Settings** (`/settings`) and enter your Langfuse host, public key, and secret key
3. Click **Save**

Once enabled, traces will be available for every completed call in Decibyl.

<Note>
Tracing activates automatically as soon as credentials are available — there is no separate enable flag to set.
</Note>
