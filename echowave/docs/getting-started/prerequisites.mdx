...
Decibyl images are published to Docker Hub:

- **Docker Hub**: `decibylai`

Override it with the `REGISTRY` environment variable if you mirror the images elsewhere:

<CodeGroup>
```bash
REGISTRY=decibylai ./scripts/start_docker.sh
```
```powershell Windows
$env:REGISTRY = 'decibylai'; .\scripts\start_docker.ps1
```
</CodeGroup>
