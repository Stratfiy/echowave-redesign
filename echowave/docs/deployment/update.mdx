...
```bash
cd decibyl
chmod +x scripts/update_remote.sh
...
```
```powershell Windows
docker compose down; .\scripts\start_docker.ps1
```
</CodeGroup>
...
<Warning>
Rolling back across a database migration is not always safe — if the newer release ran a schema migration, downgrading may leave the DB in a state the older API doesn't understand. If in doubt, [open an issue](mailto:support@decibyl.com) before rolling back.
</Warning>
