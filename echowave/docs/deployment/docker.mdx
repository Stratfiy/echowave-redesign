...
From a clone of the repository, run the starter script and confirm the prompt:
...
```bash
chmod +x scripts/setup_remote.sh && sudo ./scripts/setup_remote.sh
```
...
1. Clones the repo into `decibyl/` (the script asks for an `owner/name` and `branch`).
