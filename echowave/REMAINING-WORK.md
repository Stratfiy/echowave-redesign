...
| # | What | Why |
|---|---|---|
| ~~F2~~ | ~~`test_privacy.py` flake~~ | **Fixed 21 Aug, root cause found.** `subprocessors.in_use` reads `PlatformProviderCredentialModel` with no `ORDER BY`, so Postgres makes no promise about which of two rows for one vendor comes back first. `_merge` built the sub-processor's purpose sentence incrementally — capitalising whichever component reached it first, lowercasing the second — so a vendor serving two components (Sarvam: STT+TTS) could read "Speech recognition; speech synthesis" on one request and "Speech synthesis; speech recognition" on the next, for no reason visible to a reader. That is also a real compliance-document bug, not only a test flake: the sub-processor list a DPO reads should not change wording between page loads. Fixed by no longer building the sentence incrementally — `_merge` now only accumulates which components a vendor serves, and a new `_render` builds the sentence once, in a fixed receipt order (stt, llm, tts, telephony), so the wording is fixed regardless of scan order. A regression test reverses the insertion order directly, rather than hoping to catch the flake again |
