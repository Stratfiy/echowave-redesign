...
Deploys Decibyl on Kubernetes with decomposed backend workloads (web,
...
|------------------------------|-------------|-----------------|-------|
| `decibyl-web`                 | 2 (HPA opt) | RollingUpdate   | Long-lived WS, graceful drain |
| `decibyl-arq-worker`          | 1 (HPA opt) | RollingUpdate   | Stateless |
| `decibyl-ari-manager`         | **1 fixed** | **Recreate**    | Telephony singleton |
| `decibyl-campaign-orchestrator` | **1 fixed** | **Recreate**  | Campaign singleton (in-memory locks) |
| `decibyl-ui`                  | 2 (HPA opt) | RollingUpdate   | Next.js SSR |
| `decibyl-coturn`              | 1           | Recreate        | LoadBalancer Service, port-pinned |
