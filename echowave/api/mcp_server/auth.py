...
async def authenticate_mcp_request() -> UserModel:
    """Resolve the authenticated Decibyl user for an MCP tool invocation.
    ...
    and user identifiers. `_OrgRoutingExporter` reads `decibyl.org_id`
    ...
    if span.is_recording():
        ...
        # Intentionally NOT `decibyl.org_id` — that attribute triggers the
