...
def upgrade() -> None:
    ...
    sa.Column('status', sa.String(length=24), server_default='not_started', nullable=False),
    ...
    op.create_index('ix_organization_kyc_status_submitted', 'organization_kyc', ['status', 'submitted_at'], unique=False)
    ...
    sa.ForeignKeyConstraint(['organization_id'], ['organizations.id'], ondelete='CASCADE'),
    sa.ForeignKeyConstraint(['organization_kyc_id'], ['organization_kyc.id'], ondelete='CASCADE'),
    sa.ForeignKeyConstraint(['uploaded_by'], ['users.id'], ondelete='SET NULL'),
    sa.PrimaryKeyConstraint('id')
    )
    op.create_index('ix_kyc_documents_kyc', 'kyc_documents', ['organization_kyc_id'], unique=False)
