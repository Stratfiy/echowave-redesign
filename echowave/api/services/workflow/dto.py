...
@node_spec(
    name="handoff",
    display_name="Hand to another agent",
    description="Pass the call to one of your other agents, mid-conversation.",
    llm_hint=(
        "Hands the live call over to another agent in the same account. That "
        "agent's conversation is spliced into this one when the call starts, "
        "so from the caller's side it is one continuous call — no ringing, no "
        "hold, no second greeting.\n\n"
        "Use it when a business has genuinely different jobs and the second "
        "one is worth maintaining on its own: a receptionist that hands to a "
        "billing agent, an ordering agent that hands to a returns agent. The "
        "point is reuse — build the specialist once and put it behind several "
        "front doors.\n\n"
        "**It is a transfer, not a detour.** The call continues with the other "
        "agent and does not come back, so this node has nothing after it. If "
        "the conversation needs to return, it is a branch in one agent rather "
        "than a handoff to another.\n\n"
        "The other agent's voice and model are not adopted — the call keeps "
        "the one it started with. What is reused is the conversation: its "
        "prompts, its steps, its tools and its own global instructions."
    ),
    category=NodeCategory.call_node,
    icon="Users",
    examples=[
        NodeExample(
            name="reception_hands_billing_over",
            data={
                "name": "To billing",
                "agent_uuid": "7f3a1c02-9d5e-4a11-b8c3-2e6f4a9d1b77",
            },
        ),
    ],
    graph_constraints=GraphConstraints(min_incoming=1, max_outgoing=0),
    property_order=("name", "agent_uuid"),
    field_overrides={
        "name": {
            "spec_default": "Hand over",
            "description": "Short identifier shown in the canvas and call logs.",
        },
        "agent_uuid": {
            "display_name": "Agent",
            "description": (
                "Which of your agents takes the call from here. It must belong "
                "to this account, and the two must not hand back to each other."
            ),
        },
    },
)
class HandoffNodeData(BaseNodeData):
    ...
    def _an_agent_is_chosen(self):
        """A handoff with nothing chosen is a dead end.

        The runtime never sees a handoff node — assembly replaces it before the
        graph is built — so one that cannot be replaced would leave the caller
        at a step with nowhere to go. Caught at save rather than at dial.
        """
        if not (self.agent_uuid or "").strip():
            raise ValueError(
                "Hand-over node has no agent chosen. Pick the agent that should "
                "take the call, or remove the step."
            )
        return self
...
class HandoffRFNode(_RFNodeBase):
...
_CORE_NODE_DATA_CLASSES: dict[str, type[BaseNodeData]] = {
    ...
    NodeType.handoff.value: HandoffNodeData,
}
