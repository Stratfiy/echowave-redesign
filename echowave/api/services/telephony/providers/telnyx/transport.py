...
async def create_transport(
    ...
    *,
    ...
    # not Decibyl's: ``inbound_encoding`` is what we *send into the call*
    # (Decibyl → Telnyx), and ``outbound_encoding`` is what we *receive out of
    # the call* (Telnyx → Decibyl).
    serializer = TelnyxFrameSerializer(
        stream_id=stream_id,
        call_control_id=call_control_id,
        api_key=api_key,
        inbound_encoding="PCMU",  # Decibyl → Telnyx; matches stream_bidirectional_codec
        outbound_encoding=encoding,  # Telnyx → Decibyl; from media_format.encoding
