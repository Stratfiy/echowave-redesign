...
``lock_organization_ledger`` is ``SELECT ... FOR UPDATE`` on the organisation
row, held until the transaction ends. It is the pattern ``reservations.reserve``
already used, lifted out so every writer takes the same lock on the same row:
