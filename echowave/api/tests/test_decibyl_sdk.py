"""Tests for the Python runtime SDK (`decibyl_sdk`).
...
import pytest
from decibyl_sdk import Workflow
from decibyl_sdk._generated_models import NodeSpec
from decibyl_sdk.errors import ValidationError
...
class _StubClient:
    """Stand-in for DecibylClient backed by the in-process spec registry.
...
def test_webhook_complex_fields_validate(client: _StubClient):
    ...
    wh = wf.add(
        ...
        custom_headers=[{"key": "X-Source", "value": "decibyl"}],
