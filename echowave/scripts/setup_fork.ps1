...
$UpstreamUrl    = 'https://github.com/decibyl-hq/decibyl.git'
$CanonicalHttps = $UpstreamUrl
$CanonicalSsh   = 'git@github.com:decibyl-hq/decibyl.git'
...
Write-Host '|              Decibyl Contributor Bootstrap                    |' -ForegroundColor Blue
Write-Host '+==============================================================+' -ForegroundColor Blue
Write-Host ''
...
if ($needsForkPrompt) {
    Write-Host 'Enter your fork URL (e.g. https://github.com/<YOUR_HANDLE>/decibyl.git):' -ForegroundColor Yellow
...
if (-not $existingUpstream) {
    ...
    $reset = (Read-Host 'Reset upstream to decibyl-hq/decibyl? [y/N]').Trim()
    if ($reset -match '^[Yy]') {
