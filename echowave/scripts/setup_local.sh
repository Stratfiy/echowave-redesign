...
# Download compose file (skip when DECIBYL_SKIP_DOWNLOAD=1 — e.g. local repo testing).
TOTAL_STEPS=2
...
if [[ "${DECIBYL_SKIP_DOWNLOAD:-}" != "1" ]]; then
    ...
    curl -sS -o docker-compose.yaml https://raw.githubusercontent.com/decibyl-hq/decibyl/main/docker-compose.yaml
    if [[ "${ENABLE_COTURN:-false}" == "true" ]]; then
        decibyl_download_init_support_bundle "$(pwd)" "main"
    fi
...
if [[ "${ENABLE_COTURN:-false}" == "true" ]]; then
    [[ -f scripts/run_decibyl_init.sh ]] || decibyl_fail "scripts/run_decibyl_init.sh not found. Re-run setup_local.sh without DECIBYL_SKIP_DOWNLOAD=1, or use a full repo checkout."
    [[ -f scripts/lib/setup_common.sh ]] || decibyl_fail "scripts/lib/setup_common.sh not found. Re-run setup_local.sh without DECIBYL_SKIP_DOWNLOAD=1, or use a full repo checkout."
    [[ -f deploy/templates/turnserver.remote.conf.template ]] || decibyl_fail "deploy/templates/turnserver.remote.conf.template not found. Re-run setup_local.sh without DECIBYL_SKIP_DOWNLOAD=1, or use a full repo checkout."
fi
...
echo ""
if [[ "${ENABLE_COTURN:-false}" == "true" ]]; then
    echo -e "${YELLOW}To start Decibyl with TURN, run:${NC}"
    echo ""
