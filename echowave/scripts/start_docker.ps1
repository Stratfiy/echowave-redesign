...
function New-MinioRootUser {
    return "decibyl$((New-HexSecret).Substring(0, 12))"
}
...
function Sync-PostgresPassword {
    ...
    "ALTER USER postgres WITH PASSWORD :'decibyl_password';" | docker compose exec -T postgres psql `
        -U postgres `
        -d postgres `
        -v 'ON_ERROR_STOP=1' `
        -v "decibyl_password=$Password" > $null
    if ($LASTEXITCODE -ne 0) {
...
$answer = Read-Host 'Start Decibyl now? [Y/n]'
if ($answer -match '^[Nn]') {
    Write-Host 'Decibyl was not started.'
    exit 0
}
