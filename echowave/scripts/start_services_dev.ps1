#!/usr/bin/env pwsh
# Start Decibyl services in development mode (Windows)
...
$EnvFile    = if ($env:DECIBYL_ENV_FILE) { $env:DECIBYL_ENV_FILE } else { Join-Path $BaseDir 'api/.env' }
...
Write-Host "Starting Decibyl Services (DEV MODE) in BASE_DIR: $BaseDir"
