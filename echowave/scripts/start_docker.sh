...
sync_postgres_password() {
    ...
    printf '%s\n' "ALTER USER postgres WITH PASSWORD :'decibyl_password';" \
        | docker compose exec -T postgres psql \
        -U postgres \
        -d postgres \
        -v ON_ERROR_STOP=1 \
        -v "decibyl_password=$postgres_password" >/dev/null
...
if [[ ! -t 0 ]]; then
    echo "Run the command above from an interactive shell to start Decibyl."
    exit 0
fi

read -r -p "Start Decibyl now? [Y/n]: " answer
case "$answer" in
    [Nn]*)
        echo "Decibyl was not started."
        exit 0
        ;;
esac
