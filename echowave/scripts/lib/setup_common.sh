...
decibyl_render_remote_nginx_conf() {
...

    {
        ...
        echo "    least_conn;"
        # Re-resolve "api" instead of pinning the address it had when nginx
        # started. Docker hands a recreated container a new IP, and nginx
        # resolves upstream hostnames once at config load, so every recreate
        # of api outside a deploy was a 502 until somebody restarted nginx.
        # "resolve" needs a shared zone, and 127.0.0.11 is Docker's embedded
        # DNS. Open-source nginx from 1.27.3.
        echo "    zone decibyl_api 64k;"
        echo "    resolver 127.0.0.11 valid=10s ipv6=off;"
        for ((i=0; i<FASTAPI_WORKERS; i++)); do
            printf '    server api:%d max_fails=3 fail_timeout=10s resolve;\n' "$((8000 + i))"
        done
        echo "    keepalive 32;"
        echo "}"
    } > "$tmp_upstream"
...
decibyl_render_subdomain_nginx_conf() {
    # Render the four-hostname topology: an empty apex, the app, the API and
    # the docs, each with its own server block.
    #
    # Sibling of decibyl_render_remote_nginx_conf, not a replacement — the
    # single-host template is still the right choice for a self-hosted install
    # on one name. run_decibyl_init.sh picks between them.
    #
    # This exists because the subdomain split used to be a paragraph in the
    # deployment guide telling an operator to substitute hostnames by hand.
    # Nobody did, so every hostname landed on one default server block: the
    # docs host served the dashboard and redirected to a login page, and the
    # API and app were never actually separated. A render that happens on
    # `docker compose up` cannot be forgotten.
    local project_dir=${1:-$(decibyl_project_dir)}
    local destination=${2:-"$project_dir/nginx.conf"}
    local template=""
    local tmp_upstream=""

    local root_host="${DECIBYL_ROOT_HOST:-}"
    local app_host="${DECIBYL_APP_HOST:-}"
    ...
    awk \
        -v root_host="$root_host" \
        -v app_host="$app_host" \
        -v api_host="$api_host" \
        -v docs_host="$docs_host" \
        -v upstream_file="$tmp_upstream" '
        BEGIN {
            while ((getline line < upstream_file) > 0) {
                upstream = upstream line ORS
            }
        ...
        {
            gsub(/__DECIBYL_ROOT_HOST__/, root_host)
            gsub(/__DECIBYL_APP_HOST__/, app_host)
            gsub(/__DECIBYL_API_HOST__/, api_host)
            gsub(/__DECIBYL_DOCS_HOST__/, docs_host)
            if ($0 == "__DECIBYL_UPSTREAM_BLOCK__") {
                printf "%s", upstream
            } else {
                print
