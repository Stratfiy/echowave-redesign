...
echo -e "${BLUE}"
echo "╔══════════════════════════════════════════════════════════════╗"
echo "║              Decibyl Custom Domain Setup                      ║"
...
if [[ -z "$SERVER_IP" ]]; then
    decibyl_warn "Warning: Could not detect server's public IP"
...
decibyl_install_certbot || decibyl_fail "Could not install certbot. Please install it manually and re-run."
...
decibyl_require_init_compose_layout "$(pwd)"

decibyl_load_env_file .env
if [[ -z "${SERVER_IP:-}" ]]; then
    SERVER_IP="$(decibyl_infer_server_ip "$(pwd)" || true)"
fi
[[ -n "${SERVER_IP:-}" ]] || decibyl_fail "Could not determine SERVER_IP from the existing install"

decibyl_set_env_key .env SERVER_IP "$SERVER_IP"
decibyl_set_env_key .env PUBLIC_HOST "$DOMAIN_NAME"
decibyl_set_env_key .env PUBLIC_BASE_URL "https://$DOMAIN_NAME"
decibyl_delete_env_key .env BACKEND_URL
...
decibyl_delete_env_key .env BACKEND_API_ENDPOINT
decibyl_delete_env_key .env MINIO_PUBLIC_ENDPOINT
decibyl_delete_env_key .env TURN_HOST
decibyl_prepare_remote_install "$(pwd)"

# Bring the stack up (recreating it) so decibyl-init re-renders nginx with the
...
[[ "$nginx_ready" == "1" ]] || decibyl_fail "nginx did not come up on port 80; cannot run the ACME challenge."
...
if ! decibyl_issue_letsencrypt_webroot "$(pwd)" "$DOMAIN_NAME" "$EMAIL_ADDRESS"; then
...
decibyl_install_cert_renewal_hook "$(pwd)" "$DOMAIN_NAME"
...
echo -e "  Certificate: $DECIBYL_PATH/certs/local.crt"
echo -e "  Private Key: $DECIBYL_PATH/certs/local.key"
