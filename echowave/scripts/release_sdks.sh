#!/usr/bin/env bash
# Cut a release of both SDKs — decibyl-sdk (PyPI) and @decibyl/sdk (npm) —
...
#   - `npm login` completed as a member of the `decibyl` npm org. npm
...
if ! NPM_USER="$(npm whoami 2>/dev/null)"; then
    ...
    echo "       decibyl org before re-running this script — otherwise PyPI" >&2
...
if ! git diff --quiet -- sdk/python/src/decibyl_sdk/typed sdk/typescript/src/typed; then
    echo
...
echo
echo "============================================================"
echo "  Built decibyl-sdk==$VERSION and @decibyl/sdk@$VERSION"
echo "  Nothing has been published yet."
echo "============================================================"
echo

if confirm "Upload decibyl-sdk==$VERSION to TestPyPI first (recommended)?"; then
    (cd sdk/python && twine upload --repository testpypi dist/*)
    echo "  → https://test.pypi.org/project/decibyl-sdk/$VERSION/"
    echo
fi

if confirm "Publish @decibyl/sdk@$VERSION to npm? (will prompt for 2FA OTP)"; then
    (cd sdk/typescript && npm publish --access public)
    echo "  → https://www.npmjs.com/package/@decibyl/sdk/v/$VERSION"
    echo
fi

if confirm "Upload decibyl-sdk==$VERSION to PyPI?"; then
    (cd sdk/python && twine upload dist/*)
    echo "  → https://pypi.org/project/decibyl-sdk/$VERSION/"
    echo
fi

if confirm "Create annotated git tag sdks-v$VERSION at HEAD?"; then
    git tag -a "sdks-v$VERSION" -m "decibyl-sdk + @decibyl/sdk $VERSION"
