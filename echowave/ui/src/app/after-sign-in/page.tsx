...
export default async function AfterSignInPage() {
  ...
  logger.debug("[AfterSignInPage] Got user:", {
    hasUser: !!user,
    userId: user?.id,
  });
  ...
  if (authProvider === "stack" && user && "getAuthJson" in user) {
    logger.debug(
      "[AfterSignInPage] Stack user detected, getting auth token...",
    );
    const token = await user.getAuthJson();
    logger.debug("[AfterSignInPage] Got token:", {
    ...
    const permissions =
      "listPermissions" in user && "selectedTeam" in user
        ...
        : [];
    logger.debug("[AfterSignInPage] Got permissions:", {
  ...
  logger.debug(
    "[AfterSignInPage] Checking for existing workflows before fallback",
  );
  ...
  try {
    ...
    if (accessToken) {
      ...
      if (countResponse.data && countResponse.data.active > 0) {
        logger.debug(
          "[AfterSignInPage] Redirecting to /overview - user has workflows",
        );
        redirect("/overview");
      } else {
        // No agent yet: the first-agent journey, not the wizard.
        logger.debug(
          "[AfterSignInPage] Redirecting to /start - no workflows found",
        );
        redirect("/start");
      }
    }
